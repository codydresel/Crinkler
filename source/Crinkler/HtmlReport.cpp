...
function initialize() {
	if (INITIAL_THEME == 'AUTO' && window.matchMedia && window.matchMedia('(prefers-color-scheme: light)').matches) {
		document.body.classList.add('light');
	}
	stateGlobals();
	buildByteIndex();
...
static void HtmlReportRecursive(CompressionReportRecord* csr, back_insert_iterator<vector<char>> out, Hunk& hunk, Hunk& untransformedHunk, const int* sizefill, bool iscode, map<int, Symbol*>& relocs, map<int, Symbol*>& symbols, map<string, int>& opcodeCounters,
	...
	if(csr->type & RECORD_ROOT) {
		...
		format_to(out, "const INITIAL_THEME = '{}';\n", ReportThemeName(crinkler->GetReportTheme()));
		...
		string lightClass = crinkler->GetReportTheme() == REPORT_THEME_LIGHT ? " class='light'" : "";
		format_to(out, "<body onload='initialize()'{}>", lightClass);
		format_to(out, "<h1>Crinkler compression report</h1>");
